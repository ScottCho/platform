...
bug_ass_baseline = db.Table('bug_ass_baseline',
    db.Column('baseline_id', db.Integer, db.ForeignKey(
        'baselines.id'), primary_key=True),
    db.Column('bug_id', db.Integer, db.ForeignKey(
        'issue_bug.id'), primary_key=True)
)

requirement_ass_baseline = db.Table('requirement_ass_baseline',
    db.Column('baseline_id', db.Integer, db.ForeignKey(
        'baselines.id'), primary_key=True),
    db.Column('requirement_id', db.Integer, db.ForeignKey(
        'issue_requirement.id'), primary_key=True)
)

task_ass_baseline = db.Table('task_ass_baseline',
    db.Column('baseline_id', db.Integer, db.ForeignKey(
        'baselines.id'), primary_key=True),
    db.Column('task_id', db.Integer, db.ForeignKey(
        'issue_task.id'), primary_key=True)
)
