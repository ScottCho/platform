...
def get_project():
    ...
    if token:
        if validate_token(token):
            ...
            project_id = redis_cli.get(key)
            if project_id:
                project = Project.query.get(int(project_id.decode('utf-8')))
...
patch_request_class(flask_app)  # 文件大小限制，默认为16MB
...
@flask_app.route('/test12', methods=['GET', 'POST'])
def upload_files():
